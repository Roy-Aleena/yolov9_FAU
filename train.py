...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    nc = 5 if not single_cls else int(data_dict['nc'])  # number of classes, changed to 5
    names = data_dict['names'] if not single_cls or len(data_dict['names']) != 1 else {0: 'item'}  # class names
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Log progress
            if RANK in {-1, 0}:
                pbar.set_description(('%11s' * 7) % (f'{epoch}/{epochs}', '%.3g' % (torch.cuda.memory_allocated() / 1E9),
                                                      '%.3g' % mloss[0], '%.3g' % mloss[1], '%.3g' % mloss[2],
                                                      '%.3g' % len(imgs), '%.3g' % imgs.shape[-1]))

        # End of epoch actions
        callbacks.run('on_train_epoch_end', epoch, results)
...
def end_training(log_results=True):
    if RANK in {-1, 0}:
        ...
        for f in [last, best]:
            if f.exists():
                strip_optimizer(f, last_striped if f is last else best_striped)  # Strip optimizers
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = validate.run(
                        ...
                        callbacks=callbacks,
                        compute_loss=compute_loss
                    )  # Validate best model with plots
                    if is_coco:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='yolo.yaml', help='Path to model.yaml')
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='Path to dataset.yaml')
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-low.yaml', help='Path to hyperparameters')
    parser.add_argument('--epochs', type=int, default=100, help='Total number of training epochs')
    parser.add_argument('--batch-size', type=int, default=16, help='Batch size for all GPUs, -1 for autobatch')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=640, help='Train and validation image size (pixels)')
    ...
    parser.add_argument('--device', default='', help='CUDA device (e.g., 0,1,2,3 or cpu)')
    parser.add_argument('--multi-scale', action='store_true', help='Vary image size +/- 50%')
    parser.add_argument('--single-cls', action='store_true', help='Train multi-class data as single-class')
    parser.add_argument('--optimizer', type=str, choices=['SGD', 'Adam', 'AdamW', 'LION'], default='SGD', help='Optimizer choice')
    parser.add_argument('--sync-bn', action='store_true', help='Use SyncBatchNorm (available in DDP mode)')
    parser.add_argument('--workers', type=int, default=8, help='Max dataloader workers (per RANK in DDP mode)')
    parser.add_argument('--project', default=ROOT / 'runs/train', help='Project directory to save results')
    parser.add_argument('--name', default='exp', help='Name of the experiment')
    parser.add_argument('--exist-ok', action='store_true', help='Do not increment project/name if exists')
    ...
    parser.add_argument('--flat-cos-lr', action='store_true', help='Use flat cosine learning rate scheduler')
    parser.add_argument('--fixed-lr', action='store_true', help='Use fixed learning rate scheduler')
    ...
    parser.add_argument('--bbox_interval', type=int, default=-1, help='Bounding-box image logging interval')
